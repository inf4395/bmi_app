...
Deployment-Simulation: Das Deployment wurde in allen drei Plattformen als identische Simulationen implementiert, die nur Echo-Befehle ausführen. Diese Entscheidung wurde getroffen, um eine faire Vergleichsbasis zu gewährleisten und Infrastruktur-Abhängigkeiten zu eliminieren. Dies bedeutet, dass Aspekte wie tatsächliche Deployment-Zeiten, Rollback-Funktionalität und Multi-Environment-Deployments nicht umfassend untersucht wurden. Diese methodische Entscheidung beeinflusst die Bewertung der Deployment-Funktionalität, jedoch wurde sie getroffen, um die Vergleichbarkeit der Pipeline-Performance zu maximieren, was der Hauptfokus dieser Evaluierung ist.

Docker-in-Docker-Limitationen: Die Docker-Build-Jobs wurden in allen drei Plattformen deaktiviert, um eine faire Vergleichsbasis zu gewährleisten, da Docker-in-Docker auf dem verwendeten GitLab CI Shared Runner nicht verfügbar war. Diese Entscheidung stellt eine methodische Anpassung dar, um eine vollständig faire Vergleichsbasis zu gewährleisten, nicht eine Limitation der Plattformen selbst. In anderen Umgebungen mit Docker-in-Docker-Unterstützung könnten sich die Ergebnisse unterscheiden, jedoch wurde diese Entscheidung getroffen, um die Vergleichbarkeit der Ergebnisse zu maximieren.
