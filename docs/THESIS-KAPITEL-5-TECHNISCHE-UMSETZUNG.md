...
Die Backend-Tests verwenden Jest als Test-Framework und Supertest für HTTP-Assertionen. Die Testabdeckung umfasst sieben Testsuites: auth.test.js für Authentifizierung, bmi.test.js für BMI-Berechnungen, stats.test.js für Statistiken, calculateBMI.test.js für die BMI-Berechnungsfunktion, validation.test.js für Eingabevalidierung, security.test.js für Sicherheitstests und performance.test.js für Performance-Tests. Die Tests decken alle API-Endpunkte ab, einschließlich Authentifizierung, BMI-Berechnung, Statistiken und Fehlerbehandlung. Die Tests verwenden in-memory SQLite-Datenbanken, um Isolation zu gewährleisten und Test-Performance zu optimieren. Die Testabdeckung umfasst Unit-Tests für einzelne Funktionen und Integrationstests für API-Endpunkte.
...
Die CI/CD-Pipelines für die drei Plattformen wurden so konfiguriert, dass sie identische Aufgaben ausführen, um eine faire Vergleichsbasis zu gewährleisten. Alle Pipelines durchlaufen die gleichen Stages: Lint, Test, Build, E2E-Tests und Deploy. Die Konfigurationen wurden sorgfältig angepasst, um funktionale Äquivalenz zu erreichen, während plattformspezifische Syntax und Features genutzt werden.
...
Die GitHub Actions Pipeline wird durch eine YAML-Datei im `.github/workflows/` Verzeichnis definiert. Die Workflow-Definition verwendet die GitHub Actions Syntax mit Jobs, Steps und Actions. Die Pipeline besteht aus acht Jobs: backend-lint, frontend-lint, backend-test, frontend-test, build-frontend, e2e-tests, docker-build und deploy-staging/production.
...
graph TD
    ...
    I --> J[Start Frontend]
    J --> K[Run E2E Tests]
    K --> L[docker-build Job]
    L --> M{Branch?}
    M -->|develop| N[deploy-staging]
    M -->|main| O[deploy-production]
```
...
Die Deploy-Jobs sind als manuelle Jobs konfiguriert, die nur auf bestimmten Branches ausgeführt werden. Die Jobs verwenden alpine:latest als minimales Image und simulieren Deployment-Befehle. Die Pipeline-Konfiguration nutzt GitLab-spezifische Features wie Environment-Management, Coverage-Parsing und Artifact-Management.
...
graph TD
    ...
    J --> K[e2e-tests Job]
    K --> L[deploy Stage]
    L -->|Manual| M[deploy-staging]
    L -->|Manual| N[deploy-production]
```
...
Die Deploy-Stage verwendet input-Schritte für manuelle Deployment-Bestätigung, ähnlich wie GitLab CI manuelle Jobs. Die Stage unterscheidet zwischen Staging- und Produktions-Deployments basierend auf der Branch. Die Pipeline verwendet post-Blöcke für Cleanup-Operationen und Notifications.
...
graph TD
    ...
    J --> K[Start Servers]
    K --> L[Run Playwright]
    L --> M[Docker Build Stage]
    M --> N{Branch?}
    N -->|main/develop| O[Docker Build]
    O --> P[Deploy Stage]
    P -->|Manual Input| Q[Deploy Staging/Production]
```

Die drei Pipeline-Konfigurationen sind funktional äquivalent und führen die gleichen Aufgaben aus, ermöglichen jedoch plattformspezifische Optimierungen und Features. Die Konfigurationen dienen als Grundlage für die Performance-Evaluierung und den Vergleich der drei CI/CD-Plattformen.
